...
class UIManager {
  ...
  renderTasks() {
    ...
    const THRESHOLD = 200; // start batching beyond this
    const BATCH_SIZE = 50;
    const total = tasks.length;
    taskList.innerHTML = '';
    if (total <= THRESHOLD) {
      // Simple full render
      taskList.innerHTML = tasks.map(t=>this.renderTask(t)).join('');
    ...
    const progressBarId = 'taskRenderProgress';
    let progress = document.getElementById(progressBarId);
    if (!progress) {
      progress = document.createElement('div');
      progress.id = progressBarId;
      progress.setAttribute('aria-label','Rendering tasks');
      progress.style.cssText = 'position:sticky;top:0;left:0;height:3px;background:var(--bg-tertiary);overflow:hidden;margin-bottom:4px;border-radius:2px;';
      const inner = document.createElement('div');
      inner.style.cssText='height:100%;width:0;background:var(--primary-color);transition:width .15s linear;';
      inner.id = progressBarId+'Inner';
      progress.appendChild(inner);
      taskList.parentNode.insertBefore(progress, taskList);
    }
    const inner = document.getElementById(progressBarId+'Inner');
    const renderBatch = () => {
      const slice = tasks.slice(index, index + BATCH_SIZE);
      slice.forEach(task => {
      ...
      index += slice.length;
      inner.style.width = ((index/total)*100).toFixed(1)+'%';
      if (index < total) {
  ...
  renderTask(task) {
    ...
    if (task.relations && task.relations.length) {
      const names = task.relations.map(id => {
        const r = this.app.tasks.find(t=>t.id===id);
        return r ? r.title : 'Unknown';
      });
      const tooltip = names.join(', ');
      relationsBlock = `<div class="task-relations" title="${this.escapeHtml(tooltip)}"><small>Related: ${task.relations.length}</small></div>`;
    }

    return `
      <div class="task-item ${task.completed ? 'completed' : ''} ${isOverdue ? 'overdue' : ''}" 
           data-task-id="${task.id}" draggable="true" role="listitem" aria-label="Task ${this.escapeHtml(task.title)}">
  ...
  setupTaskEventListeners() {
    ...
    // Focus mode open on title single click (not editable state)
    document.querySelectorAll('.task-title').forEach(title => {
      title.addEventListener('click', (e) => {
        if (title.contentEditable === 'true') return; // ignore when editing inline
        const taskId = e.target.closest('.task-item').dataset.taskId;
        this.openFocusMode(taskId);
      });
    });
  ...
  renderFocus(taskId) {
    ...
    const relEl = document.getElementById('focusRelations');
    titleEl.textContent = task.title;
    const chips = [];
    if (task.priority) chips.push(`<span class="chip priority-${task.priority}">${task.priority}</span>`);
    if (task.dueDate) chips.push(`<span class="chip">Due ${task.getDueDateFormatted()||''}</span>`);
    if (task.reminderAt && !task.reminderDismissed) chips.push(`<span class="chip">Reminder</span>`);
    if (task.tags && task.tags.length) chips.push(...task.tags.map(t=>`<span class="chip">#${t}</span>`));
    metaEl.innerHTML = chips.join('');
    descEl.textContent = task.description || 'No description';
    if (task.relations && task.relations.length) {
      const names = task.relations.map(id => {
        const r = this.app.tasks.find(x=>x.id===id); return r ? r.title : 'Unknown';
      });
      relEl.innerHTML = `<strong>Related:</strong> ${names.map(n=>`<a href="#" data-rel-task="${n}">${this.escapeHtml(n)}</a>`).join(', ')}`;
    } else { relEl.innerHTML = ''; }
    // Buttons
    document.getElementById('focusToggleComplete').textContent = task.completed ? 'Mark Incomplete' : 'Mark Complete';
  }
  ...
  populateTaskForm(task) {
    ...
    if (task.reminderAt) {
      ...
      const local = new Date(rdate.getTime() - rdate.getTimezoneOffset() * 60000).toISOString().slice(0,16);
      document.getElementById('taskReminder').value = local;
    } else {
